...
def test_get_all_paginated_items(git_data_collector):
    ...
    with requests_mock.Mocker() as m:
        m.get("https://api.github.com/items?page=1", json=[{"item": "data1"}], headers={'Link': '<https://api.github.com/items?page=2>; rel="next"'})
        m.get("https://api.github.com/items?page=2", json=[{"item": "data2"}])
        
        items = git_data_collector.get_all_paginated_items("https://api.github.com/items?page=1")
        assert len(items) == 2
...
def test_create_dataframe_with_prs(git_data_collector):
    ...
    with requests_mock.Mocker() as m:
        ...
        m.get("https://api.github.com/orgs/testOrg/repos?per_page=5", json=[{
            "name": "repo1",
            "owner": {"login": "testOwner"},
        }])
        m.get("https://api.github.com/repos/testOwner/repo1/pulls?state=all&per_page=5", json=[{
            "id": "pr1",
            "state": "open",
            "title": "Test PR",
            "user": {"login": "user1"},
